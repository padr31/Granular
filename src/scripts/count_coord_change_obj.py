...
#Code that turns all the objects to the chosen one
def change(group_name, obj_name):
    linked = False   

    chosen = bpy.data.objects[obj_name]
...
group_name = "Group"
obj_name = "Template"
#change(group_name, obj_name)

g = bpy.data.groups[group_name]
count = 0
num = 0
numa = 0
icount = 0
...
for i in g.objects:
    ...
    for j in g.objects:
        if i == j:
            continue
        touch = bmesh_check_intersect_objects(i, j)
        icount += touch
        count+= touch
    if not(icount == 0 or icount == 1):
        sys.stdout.write(str(icount) + ", ") 
    if icount == 0 or icount == 1:
        count -= icount
        num-=1   
        
print("\n" + str(count))
print(num)        
print("True amount: " + str(numa))
print((count/(num)))